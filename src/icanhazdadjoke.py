...
class Icanhazdadjoke:
	...
	def get_random_dad_joke(self):
		return get(
			f"{self.api}/", headers=self.headers).json()
	
	def get_random_dad_joke_as_slack(self):
		return get(
			f"{self.api}/slack",
			headers=self.headers).json()
	
	def get_dad_joke(self, joke_id: str):
		return get(
			f"{self.api}/j/{joke_id}",
			headers=self.headers).json()
	
	def get_dad_joke_as_image(self, joke_id: str):
